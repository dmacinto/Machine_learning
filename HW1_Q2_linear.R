...
real <- transform(real, y_real = 1.8 * x_real + 2)
...
train <- transform(train, y_train = 1.8 * x + 2 + error)
...
test <- transform(test, y_test = 1.8 * x + 2 + error)
...
#MSE for kNN
for (i in k_array){
  k_near = kknn(y_train ~ x, train, test, k = i, kernel = "rectangular")
...
plot(log_k, kNN_MSE, ylim = c(0, 0.02))
